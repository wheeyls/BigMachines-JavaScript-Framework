 /**
 * Support for the BigMachines JavaScript Framework
 * Edit "bm-framework.js" to activate the framework.
...
(function() { 
	var setup = window["bm-framework/setup"], me = {}, ps = {}, log = {}, debug = false;
	if(!setup) {return;}
	...
	me.begin = function() {
		...
		for(i=0, ii = testees.length; i<ii; i++) {
			if(testees[i].active !== true) {continue;}
			curr_page = me.pages[testees[i].name];
			if(!curr_page) {
				throw new Error("Problem initiating BigMachines JavaScript framework. File "+ testees[i].name + " has not been configured.");
			}

			me.search_for(testees[i].name, curr_page);
		}
